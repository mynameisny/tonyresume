            ...
            <?php while ( have_posts() ) : the_post(); ?>
				...
				<section class="section post_section blog_section">
					...
					<?php if ($post_type == 'post') { ?>
						<h2 class="section_title post_section_title"><strong><span class="icon <?php echo $post_icon; ?>"></span><span class="section_name"><?php echo date('M.d', strtotime($post_date)); ?></span></strong></h2>
					<?php if (!$post_protected) { ?>
						<div class="post-info">
							<a href="<?php echo $author_link; ?>" class="post_author"><span class="icon icon-user"></span><?php echo $author_name; ?></a>
							<?php if ($post_categories_str) { ?><span class="post_categories"><span class="icon-align-left"></span><?php echo $post_categories_str; ?></span><?php } ?>
                ...
                <?php if (!$post_protected) { ?>
					...
					<?php if ($post_type == 'post') {
						//===================================== Post tags list =====================================
						if (get_theme_option("blog_show_post_tags") == 'yes') {
                            ...
                            <div class="block-social">
                                ...
                                <?php
                                    showShareSocialLinks(array(
                                        ...
                                        'icon_size' => '',
                                        'allowed' => array('facebook', 'twitter', 'gplus', 'linkedin', 'dribble', 'vimeo')
                                    ));
	                            ?>
                            </div>
						<?php
							...
							<div id="related_posts">
								...
								foreach( $recent_posts as $recent ) {
									...
									<article id="post_<?php echo $recent['ID']; ?>" class="related_posts <?php echo ($i % 2 == 0 ? 'even' : 'odd') . ($i==1 ? ' first' : ''); ?>">
										...
										<div class="post-info">
											<a href="<?php echo $recent['link']; ?>" class="post_date"><span class="icon-time"></span><?php echo date('M.d', strtotime($recent['post_date'])); ?></a>
